...
class MenuView(arcade.View):
    ...
    def on_draw(self):
        ...
        arcade.draw_texture_rectangle(constants.SCREEN_WIDTH // 2, 600, 200, 100, arcade.load_texture(self.Play_Button))
            
        if self.play_hover == True:
            arcade.draw_rectangle_outline(constants.SCREEN_WIDTH // 2, 600, 200, 100, arcade.color.GRAY, 3)

    def on_mouse_press(self, _x, _y, _button, _modifiers):
        # checks for mouse press to see if the player hit the play button
        if _button == arcade.MOUSE_BUTTON_LEFT:
            if _x >= constants.SCREEN_WIDTH // 2 - 100 and _x <= constants.SCREEN_WIDTH // 2 + 100 and _y <= 650 and _y >= 550:
    ...
    def on_mouse_motion(self, x: float, y: float, dx: float, dy: float):
        ...
        if x >= constants.SCREEN_WIDTH // 2 - 100 and x <= constants.SCREEN_WIDTH // 2 + 100 and y <= 650 and y >= 550:
